...
ВНИМАНИЕ: перед выполнением тестов делается бекап базы данных, по которому потом восстанавлиется БД, которая изменяется тестами => есть вероятность потери данных при запуске данных тестов
